...
public class CustomAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    /**
     * This function gets called on authentication success and creates a token for the user while also
     * adding it to the body for the user to retrieve
     *
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response,
        ...
        String refresherToken = jwtUtility.generateRefresherToken(user,
                request.getRequestURL().toString());

        response.setContentType(APPLICATION_JSON_VALUE);

        new ObjectMapper().writeValue(response.getOutputStream(),
                new JwtResponse(accessToken, refresherToken));
    }
}
